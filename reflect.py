...
class Reflect(Scene):
    def construct(self):
        ...
        lambda1 = lambda x : -np.sin( x - a.get_value() ) if 0+a.get_value()>x>-PI+a.get_value() else 0
        lambda2 = lambda x : np.sin( x + 9*PI + a.get_value()) if 19*PI-a.get_value()>x>18*PI-a.get_value() else 0
        ...
        lambda: axis.plot(lambda x: lambda1(x) + lambda2(x)).set_color(DARKER_GRAY))

        # Group the waves        
        superpose = VGroup(axis, superposition_wave)
        sine_waves = VGroup(axis, sine_wave_1, sine_wave_2)
        waves = VGroup(sine_waves, superpose)
        # Display the axes and sine waves
        self.play(Create(superpose))
        self.play(a.animate.set_value(20 * PI), run_time=3, rate_func=rate_functions.linear)
